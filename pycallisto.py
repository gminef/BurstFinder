...
class PyCallisto:
    ...
    def spectrogram(self, option=3, xtick=2, blevel=0, end_pts=[False, False], fig_size=(10, 10), cmap=cm.jet,
                    color_bar=True, color_bar_ori='vertical', font_size=14,show_tick = False, return_plot=True):
        ...
        if option == 4: # Dont draw xticks and yticks 
  
            cax = ax.imshow(self.imageHdu.data,aspect='auto',
...
#            plt.show()
            ...
            #print("min , max freq ", min(freqs)," ",max(freqs))
            #plt.yticks(freqs)
        ...
        if (return_plot):
            return plt, self.imageHdu.data, freqs,start_time,end_time
        else:
            plt.clf()
            plt.cla()
            plt.close()
            return self.imageHdu.data, freqs,start_time,end_time
